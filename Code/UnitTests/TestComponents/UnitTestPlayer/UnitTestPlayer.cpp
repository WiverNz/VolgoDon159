...
namespace
{
	static void RegisterPlayerComponent(Schematyc::IEnvRegistrar& registrar)
	{
		...
		{
			Schematyc::CEnvRegistrationScope componentScope = scope.Register(SCHEMATYC_MAKE_ENV_COMPONENT(CUnitTestPlayerComponent));
		}
	}
...
void CUnitTestPlayerComponent::Initialize()
{
	...
	SRmi<RMI_WRAP(&CUnitTestPlayerComponent::RemoteReviveOnClient)>::Register(this, eRAT_NoAttach, false, eNRT_ReliableOrdered);
}

void CUnitTestPlayerComponent::InitializeLocalPlayer()
{
...
Cry::Entity::EventFlags CUnitTestPlayerComponent::GetEventMask() const
{
	return
...
void CUnitTestPlayerComponent::ProcessEvent(const SEntityEvent& event)
{
	switch (event.event)
	{
...
bool CUnitTestPlayerComponent::NetSerialize(TSerialize ser, EEntityAspects aspect, uint8 profile, int flags)
{
...
void CUnitTestPlayerComponent::OnReadyForGameplayOnServer()
{
	...
	SRmi<RMI_WRAP(&CUnitTestPlayerComponent::RemoteReviveOnClient)>::InvokeOnOtherClients(this, RemoteReviveParams{ playerPosition, playerRotation });
	...
	CUnitTestPlugin::GetInstance()->IterateOverPlayers([this, channelId](CUnitTestPlayerComponent& player)
	{
		...
		SRmi<RMI_WRAP(&CUnitTestPlayerComponent::RemoteReviveOnClient)>::InvokeOnClient(&player, RemoteReviveParams{ currentOrientation.t, currentOrientation.q }, channelId);
	});
}

bool CUnitTestPlayerComponent::RemoteReviveOnClient(RemoteReviveParams&& params, INetChannel* pNetChannel)
{
...
void CUnitTestPlayerComponent::Revive(const Matrix34& transform)
{
	m_isAlive = true;
...
void CUnitTestPlayerComponent::HandleInputFlagChange(const CEnumFlags<EInputFlag> flags, const CEnumFlags<EActionActivationMode> activationMode, const EInputFlagType type)
{
	switch (type)
	{
