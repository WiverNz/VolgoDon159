	...
	, m_freezeTimeOut(ParseTimeOut(cmdLine))
{
	     ...
	     pFactory = pFactory->GetNextInstance())
	{
		...
		CryLogAlways("Found factory test %s", pFactory->GetTestInfo().name);
		AddFactory(pFactory);
	}
...
void CTestSystem::Update()
{
	if (m_isRunningTest)
	{
		...
		if (!m_remainingTestFactories.empty() && !m_currentTestInstance)
		{
			...
			CryLogAlways("Test '%s' - isEnabledForGame: %d isEnabledForEditor: %d isEditor: %d", pTestFactory->GetTestInfo().name, isEnabledForGame, isEnabledForEditor, isEditor);
		...
		if (m_remainingTestFactories.empty() && !m_currentTestInstance)
		{
			if (auto pReporter = m_pReporter.lock())
			{
				CryLogAlways("pReporter->OnFinishTesting");
...
template<typename F> void CTestSystem::RunTestsFilter(F&& predicate)
{
	...
	if (!m_pReporter.lock())
	{
		...
		//m_pExcelReporter = std::make_shared<CTestExcelReporter>(gEnv->pSystem->GetILog());
	...
	stl::optional<SError> err = TryRun([&]
		{
			...
			CryLog("CTestSystem InitTest %s", m_currentTestInstance->GetTestInfo().name);
...
{
	CryLogAlways("AddFactory for '%s' with isEnabledForGame: %d and isEnabledForEditor: %d", pFactory->GetTestInfo().name, pFactory->IsEnabledForGame(), pFactory->IsEnabledForEditor());
